...
class MDepartment2sTable extends AppTable
{
    ...
    public function uniqueCode($value, array $options): bool
    {
        ...
        // $m_department1_id: 所属本部ID
        $m_department1_id = $options['data']['m_department1_id'] ?? null;

        $count = $this
            ->find('activeRecord')
            ->where(['MOrganizations.m_department1_id' => $m_department1_id])
            ->where(['code' => $value])
            ->count();
        return ($count === 0);
    }
    ...
    /**
     * 組織マスタに登録されている(削除済みでない)部店の一覧を取得する
     * 
